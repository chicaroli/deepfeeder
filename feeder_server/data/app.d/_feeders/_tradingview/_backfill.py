...
# from _feeders._tradingview._transform import df_row_to_dh_row
...
#         super().__init__(provider="_tradingview", symbols=symbols_only, key_column="Timestamp", exchange=self.exchanges, feeder_name=feeder_name)
...
#             emit_event(
#                 "feeder", f"_tradingview:{symbol}", "backfill", "INFO", "BACKFILL_INIT",
...
#                 if df.empty:
#                     emit_event(
#                         "feeder", f"_tradingview:{symbol}", "backfill", "WARN", "API_EMPTY",
...
#                 emit_event(
#                     "feeder", f"_tradingview:{symbol}", "backfill", "INFO", "INGEST",
#                     f"Ingested backfill rows for {symbol}: written={written}, skipped={skipped}",
#                     {"rows": len(df), "written": written, "skipped": skipped}
#                 )
#             except Exception as ex:
#                 emit_event("feeder", f"_tradingview:{symbol}", "backfill", "ERROR", "INGEST_ERR",
