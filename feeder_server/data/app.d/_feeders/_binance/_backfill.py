...
# from _feeders._binance.journal_adapter import ingest_rest_rows
#
#
...
#         super().__init__(provider='_binance', symbols=[symbol], key_column=key_column, exchange=exchange, api_key=api_key)
#         self.symbol = symbol
#         self.gaps_tbl = None
#
...
#             emit_event("feeder", f"_binance:{self.symbol}", "backfill", "INFO", "INGEST",
#                        f"Ingested backfill rows for {self.symbol}: written={written} failed={failed}",
#                        {"rows": len(data), "written": written, "failed": failed})
#         except Exception as ex:
#             emit_event("feeder", f"_binance:{self.symbol}", "backfill", "ERROR", "INGEST_ERR",
...
#         all_rows: List[dict] = []
#
#         emit_event("feeder", f"_binance:{self.symbol}", "backfill", "INFO", "BACKFILL_INIT",
...
#                         emit_event("feeder", f"_binance:{self.symbol}", "backfill", "WARN", "REQ_ERR",
...
#                         emit_event("feeder", f"_binance:{self.symbol}", "backfill", "WARN", "RATE_LIMIT",
...
#                         backoff = min(10, 2 ** attempt)
#                         emit_event("feeder", f"_binance:{self.symbol}", "backfill", "WARN", "SERVER_ERR",
...
#                         emit_event("feeder", f"_binance:{self.symbol}", "backfill", "ERROR", "NO_ID",
...
#
#             emit_event("feeder", f"_binance:{self.symbol}", "backfill", "INFO", "BACKFILL_DONE",
