...
# from providers._binance.schema import (
...
#             failed += 1
#             emit_event('feeder', '_binance', 'backfill', 'ERROR', 'WRITE_ERR', f'Write error: {ex}', {'error': str(ex)})
#
...
# ])
#
# # Provider-specific replay logic for _binance
...
#             except Exception:
#                 pass
#         emit_event("journal", name, "replay", "INFO", "REPLAY", f"[replay] _binance {symbol} +{n}")
#         return f"[replay] _binance {symbol} +{n}"
#     except Exception as exc:
#         emit_event("journal", name, "replay", "ERROR", "REPLAY_ERR", f"Replay error: {exc}")
#         return f"[replay] _binance {symbol} ERROR: {exc}"
