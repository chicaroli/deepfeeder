...
def quote_json_to_tick(symbol: str, q: dict) -> Tick:
    # q: {"ts": seconds, "bid":..., "ask":..., "mid": optional}
    ...
    return Tick(
        ...
        stream="quotes",
        symbol=symbol.upper(),
        ts_ns=int(q["ts"]) * 1_000_000_000,
        seq=None,
        payload={"bid": bid, "ask": ask, "mid": mid},
        is_final=False,     # provisional; API bars will finalize
    )

def api_bar_to_tick(symbol: str, bar: dict) -> Tick:
    # bar: {"t": seconds, "o":..., "h":..., "l":..., "c":..., "v":...}
    return Tick(
        provider="tradingview",
        stream="ohlcv_1m",
        symbol=symbol.upper(),
        ts_ns=int(bar["t"]) * 1_000_000_000,
        seq=None,
        payload={"open": float(bar["o"]), "high": float(bar["h"]),
